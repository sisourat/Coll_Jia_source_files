...
subroutine setup_coll(finput)
 ...
 !call system('python $QP_ROOT/plugins/nico/cippres/libs/coll_input.py '//trim(finput)//' > collinp')
 call system('python3.10 $QP_ROOT/plugins/qp_plugins_jia/cippres_20240414/libs_jia/coll_input.py '//trim(finput)//' > collinp')
 ...
 nztemp=nz+1 !!ccjia: to avoid the nz-number problem.
 call ezfio_set_cippres_n_time(nztemp)
 allocate(ztmp(1:nz+1),ttmp(1:nz+1))
 ...
 elseif(ztype=='exp' .or. ztype=='Exp' .or. ztype=='EXP') then
  ...
  spac=0d0
  do i = 1, int(DBLE(nz)/DBLE(2.0))!nz/2
   spac=spac+1.1d0**DBLE(i)
  enddo
   spac=(zmax-zmin)/(2d0*spac)

  ztmp=0.0d0 !ztmp(nz/2)=0d0
   j = 0
  do i = int(DBLE(nz)/DBLE(2.0))+2, nz !nz/2+1, nz-1
    j = j + 1
    ztmp(i) = ztmp(i-1)+1.1d0**DBLE(j)*spac
    ztmp(nz+2 -i) = -ztmp(i)
  enddo
    ztmp(1) = zmin
    ztmp(nz +1) = zmax
 ...
 if(btype=='linear' .or. btype=='Linear' .or. btype=='LINEAR') then
  ...
  do i = 1, nb
    btmp(i) = bmin + DBLE((i-1))*(bmax-bmin)/DBLE(nb)
  enddo
 ...
 write(*,*) "ccjia:ttmp",ttmp
 call ezfio_set_cippres_zgrid(ztmp)
 write(*,*) "ccjia:ztmp",ztmp
