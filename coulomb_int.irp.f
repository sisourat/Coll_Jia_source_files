...
! :math:`\langle \chi_i | -\frac{1}{|r-R_A|} | \chi_j \rangle`
  ...
  write(111,*) "I am here: output for: nucl_coord and ao_integrals_coulomb_center(ao_num,ao_num,n_coulomb_center), from coulomb_int.irp.f"
  do j = 1, ao_num
    num_A = ao_nucl(j)
    write(111,*) "j,ao_num,num_A,nucl_coord(num_A,1:3)=",j,ao_num,num_A,nucl_coord(num_A,1:3)
    do  k = 1, n_coulomb_center
      write(111,*) "j,k,ao_num,n_coulomb_center,coulomb_center(1:3,k)=",j,k,ao_num,n_coulomb_center,coulomb_center(1:3,k)
      do i = 1, ao_num
        num_B = ao_nucl(i)
        write(111,*) "j,k,i,ao_num,num_B,nucl_coord(num_B,1:3)=",j,k,i,ao_num,num_B,nucl_coord(num_B,1:3)
        do l=1,ao_prim_num(j)
           !write(111,*) "j,k,i,l,l=ao_prim_num(j)=",j,k,i,l,ao_prim_num(j)
          do m=1,ao_prim_num(i)
            !write(111,*) "j,k,i,l,m,m=ao_prim_num(i)=",j,k,i,l,m,ao_prim_num(i)
          enddo
        enddo
        write(111,*) "i,j,k,ao_integrals_coulomb_center(i,j,k)=",i,j,k,ao_integrals_coulomb_center(i,j,k)
      enddo
    enddo
  enddo
...
! where R_k is the coordinate of the k-th nucleus.
 ...
 do ii=1,mo_num
   do jj=1,mo_num
     write(112,*) "i,j,mo_integrals_coulomb_center(i,j,1)==",ii,jj,mo_integrals_coulomb_center(ii,jj,1)
   end do
 end do
